...
export default function AddStudentModal() {
  ...
  const { isAddModalOpen, closeAddModal, refreshStudents } = useStudent();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // Pass null if checkbox is unchecked
          ...
          <div className="p-4 bg-slate-50 dark:bg-zinc-800/50 rounded-lg border border-slate-200 dark:border-zinc-800">
            ...
            {!hasIepDate && (
              <p className="text-xs text-slate-400 italic">
                This student will not appear in the "Upcoming IEPs" widget.
              </p>
            )}
          </div>
